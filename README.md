...
**角色名、系列名、分類名官網完全冇翻譯，一律日文**，所以呢啲一定要靠字典。

> 順帶一提：官方譯法係台灣腔，所以商品名會出現「小八貓」「烏薩奇」，
> 而旁邊嘅角色 tag 因為過咗字典會顯示「小八」「兔兔」。想商品名都跟埋
> 香港叫法就話我知，我可以喺字典開一個「連官方名都要換」嘅小清單。
...
{
  "詞彙": {
    "掛飾公仔": ["マスコット", "マスコットぬいぐるみ", "ぬいぐるみマスコット"],
    "兔兔":     ["うさぎ", "ウサギ", "烏薩奇"]
  }
}
```

結構係 **繁中詞 : [日文寫法…]**，即係多個日文對一個繁中（多對一）。
官網同一樣嘢有幾種寫法、或者官方譯名同你順口嗰個唔同，掛埋喺同一條就得。

### 替換規則

1. 成句完全相同 → 直接換（最快，亦最準）
2. 否則將字典所有詞砌成一個 regex，**由長到短**逐個換

第二步要小心：「ステンド**グラス**風」會俾「グラス→杯」食咗一截，變成
「ステンド杯風」。解法係字典要有完整嘅長詞（`"彩繪玻璃": ["ステンドグラス"]`），
長詞行先就唔會拆錯。漏譯記錄啱啱好捉到「ステンド」提你補。
